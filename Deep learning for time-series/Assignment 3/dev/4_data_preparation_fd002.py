...
for ax, col in zip(axes, OP_COLS):
    ...
    ax.set_title(col)
    ax.set_xticks(cluster_op_means.index)
plt.tight_layout()
fig.savefig(FIGS_DIR / "FD002_cluster_op_means.png", dpi=150)
plt.show()

# %%  normalised sensor trajectories (selected sensors, a few sample engines)
with open(RESULTS_DIR / "selected_features.yaml") as f:
    selected_sensors = yaml.safe_load(f)["selected_sensors"]

sample_units = train["unit"].unique()[:N_SAMPLE]
n_cols = 3
n_rows = -(-len(selected_sensors) // n_cols)
...
fig.suptitle(
    f"Normalised sensor trajectories for {N_SAMPLE} engines — {DATASET} train split",
    y=1.01,
)
plt.tight_layout()
fig.savefig(FIGS_DIR / "FD002_normalised_sensor_trajectories.png", dpi=150, bbox_inches="tight")
plt.show()
