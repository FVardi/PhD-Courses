...
Usage:
    python 13_lstm_hparam_study.py
    python 13_lstm_hparam_study.py --dataset FD002
"""

# %%  config — edit this when running as cells
DATASET = "FD001"   # "FD001" or "FD002"
...
# CLI overrides the cell variable when running as a script
...
# %%  load modules and base config
_train_mod = _load_module("train",    "7_train.py")
_eval_mod  = _load_module("evaluate", "8_evaluate.py")
...
for approach, w, h, lr in grid:
    ...
    for seed in STUDY_SEEDS:
        ...
        if ckpt_path.exists():
            ...
            cfg["training"]["lr"]              = lr
            cfg["training"]["sequence_lr"]     = lr  # ensure lr axis is effective for sequence
